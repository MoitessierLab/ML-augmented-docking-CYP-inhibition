...
def prepare_and_run_permutation_importance(X_train, y_train, X_test, y_test, model, pm, seed):
    test_size = 1 - 0.85
    X_tune_train, _, y_tune_train, _ = train_test_split(X_train, y_train, test_size=test_size, random_state=seed,
                                                        shuffle=False)
    X_tune_test, _, y_tune_test, _ = train_test_split(X_test, y_test, test_size=test_size, random_state=seed,
                                                      shuffle=False)
...
def Do_RandomForest_FS(X, y, X_test, y_test, name, pm, scored, seed):
    ...
    X_tune_s, _, y_tune_s, _ = train_test_split(X_selected, y, test_size=test_size + 0.1, shuffle=True,
                                                random_state=seed)
    ...
    scores_prob_test = rf_selected.predict_proba(X_selected_test)
    ...
    testresults = CYP_inhibition_functions.testresults(y_test, scores_prob_test, scores_pred_test)
...
def Do_RandomForest_FS_PI(X, y, X_test, y_test, name, pm, scored, seed):
    ...
    print('2nd Feature Importance Done')
    result_train, result_test = prepare_and_run_permutation_importance(X_selected, y, X_selected_test, y_test,
                                                                       rf_selected, pm, seed)
...
def Do_XGradientBoost(X, y, testSet, scores_test, name, pm, scored, seed):
    ...
    testresults = CYP_inhibition_functions.testresults(scores_test, scores_prob_test, scores_pred_test)
...
def Do_KNN(X, y, testSet, scores_test, name, pm, scored, seed):
    ...
    if pm['hyp_tune'] == 1:
        ...
        space4knn = {
            ...
            'n_neighbors': hp.choice('n_neighbors', range(5, 301)),
...
def Do_DNN(X, y, testSet, scores_test, name, pm, scored, seed):
    ...
    scores_prob_test = hypermodel.predict(testSet_norm)
    bi = np.argmax(scores_prob_test, axis=1)
...
def Do_Ensemble(X_train, y_train, X_test, y_test, data_set_name, pm, a, seed, model_dir, isoform):
    ...
    # Get the list of model filenames in the current directory
    filenames = os.listdir(pm['dir'] + '/' + model_dir)

    # Load all the models with "3A4" in their filename and append them to model_list
    for filename in filenames:
        if isoform in filename and filename.endswith(".json"):
            print('using model: ' + filename)
            bst = xgb.Booster({'nthread': 4})  # Initiate model
            bst.load_model(pm['dir'] + '/' + model_dir + '/' + filename)  # Load model
            model_list.append(bst)

    # Create an ensemble prediction by averaging the predictions of each model
    auc_scores = []
    pred_list = []
    for model in model_list:
        preds = model.predict(xgb.DMatrix(X_test, enable_categorical=False))
        pred_list.append(preds)
        y_pred_label_individual = np.round(preds)
        auc = roc_auc_score(y_test, preds)
        auc_scores.append(auc)
        print(f'AUC for model {model}: {auc}')
    ...
    # Assuming the CYP_inhibition_functions.testresults function is properly defined
    from CYP_inhibition_functions import testresults
    testresults = testresults(y_test, y_pred_label)

    score = {**scored, **testresults}
    score = pd.DataFrame.from_dict(score, orient='index', columns=[data_set_name])
